...
class User extends BaseLogic
{
    ...
    public function getList(array $where, $order = [], $select = '*')
    {
        ...
        //获取总数
        $total = Db::table('j_oauth_login')->where($where)->count();
        ...
        if (!empty($pageSize) && $pageSize > 0) {
            if ($total > 0) {
                ...
                $data = Db::table('j_oauth_login')->limit($page * $pageSize, $pageSize)->where($where)->order($orderSort)->select();
            }
        } else {
            $data = Db::table('j_oauth_login')->where($where)->order($orderSort)->select();
        }
        return array(
            'total' => $total,
            'list'  => $data,
